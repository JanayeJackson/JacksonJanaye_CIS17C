 ...
 *
 * Created on May 28th, 2024, 1:32 AM
 */
...
map<string, int> setFMap();   //add faces and their actual values to a map
...
map<string, int> setFMap(){
    map<string, int> value = setFMap();
    value["ace"] = 1;
    value["two"] = 2;
    value["three"] = 3;
    value["four"] = 4;
...
void stats(stack<card> player, stack<card> dealer){
    ...
    cout<<"Would you like to see if a specific card was played?(y or n)";
    cin>>choice;
    if(choice == 'y' || choice == 'Y'){
        for(int i = 0; i < pSize; i++){
            display = temp.top();
            avl.root = avl.insert(avl.root, display); 
        }
        cout<<"Please enter face - enter in all lowercase"<<endl;
        cin>>pick;
        cout<<"Please enter suite - enter in all lowercase"<<endl;
        cin>>pick2;
        display.face = pick;
        display.suit = pick2;
        Node *t = avl.search(root, display);
        if(t != NULL){
            cout<<"This card was played"<<endl;
        }
        else
            cout<<"This card was not played"<<endl;
    }
    cout<<"Would you like to see a list of all cards?(y or n";
    cin>>choice;
    if(choice == 'y' || choice == 'Y'){
        
        for(int i = 0; i < pSize; i++){
        pTemp[i] = temp.top();
        p.insertItem(temp.top());
        temp.pop();
        }
        temp = dealer;
        for(int i = 0; i < dSize; i++){
            dTemp[i] = temp.top();
            d.insertItem(temp.top());
            temp.pop();
        }
