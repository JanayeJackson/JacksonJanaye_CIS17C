...
int srch1(const char sen[], const char pat[], int line)
{
    string let = pat;
    for(int i = line; i <81; i++){
        string check;
        for(int n =i; n<strlen(pat)+i; n++){
            check += sen[n];
        }
        if(check == let){
            return i;
...
void srchAll(const char sen[], const char pat[], int match[]){
    int check = 0;
    int num;
    int i = 0;
    do{
        num = srch1(sen, pat, check);
        match[i] = num;
        check = 0;
        check += num + 1;
        i++;
